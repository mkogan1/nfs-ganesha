...
static const struct timespec msg_tout = { 0, 0 };
		   ...
		   const struct avltree_node *rhs)
{
	...
	return sockaddr_cmpf(&lk->node_addr, &rk->node_addr, true);
}
...
/**
 * This function  gets list of all ipaddresses set for the this node, and
 * compares with the input ip address given. If the input Ip address present
 * in the list, then it returns true. Basically it checks if given ip
 * belong to local system or not.
 *
 * @param [in] ipv4 or ipv6 address of the ceph nodes in string format
 * @return  true if ip belong to local system
 */
bool is_self_address(const char *in_addr)
{
...
static bool cqos_send_rpc_msg(CLIENT *clnt, cluster_qos_msg msg)
{
	...
	/*
	 * reply timeout also treated as success, as we dont expect any reply
	 * from other nodes.
	 */
				  ...
				  qos_class_type_t class_type)
{
	...
	if (bucket->iops_subscribed == 1) {
		return;
	}

	bucket->iops_subscribed = 1;
...
static void send_publish_message(sockaddr_t sockaddr, cluster_qos_msg cqos_msg)
{
	...
	if (!glist_empty(&cqos_hosts)) {
		...
		glist_for_each(glist, &cqos_hosts) {
			...
			if (node == NULL)
				return;
			if (!sockaddr_cmpf(&node->node_addr, &sockaddr, true)) {
				cqos_process_send_msg(&node->fd, &node->clnt,
						      node->node_addr,
						      cqos_msg);
				return;
...
/**
 * This function papulates all write op_type info, for bw/iops
